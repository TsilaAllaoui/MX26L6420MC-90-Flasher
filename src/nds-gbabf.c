...
{
	u16 value = GBAROM[address];
	iprintf("0x%08lx --> 0x%04x\n", address, value);
	return value;
}

u16 read_swapped_word_rom(unsigned long address)
{
	u16 value = swapBits(read_word_rom(address));
	iprintf("(swapped) 0x%08lx --> 0x%04x\n", address, value);
	return value;
}
...
{
	u16 swapped = swapBits(byte);
	write_word_rom(address, swapped);
	iprintf("(swapped) 0x%08lx <-- 0x%04x | 0x%04x\n", address, byte, swapped);
}
...
void erase_MSP55LV128()
{
	...
	do
	{
		...
		swiDelay(20);
	} while (status != 0xFFFF); // ((status | 0xFF7F) != 0xFFFF);
...
int main()
{
	...
	consoleInit(&bottomScreen, 3, BgType_Text4bpp, BgSize_T_256x256, 31, 1, false, true);
